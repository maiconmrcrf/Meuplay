import os, sys, json, logging, urllib3, time, re, threading
    ...
    for var_canal in variacoes:
                ...
                if time.time() - timestamp < TEMPO_CACHE_SELENIUM:
                    try:
                        ...
                        if r.status_code == 200 and ("#EXTM3U" in r.text or "#EXT-X" in r.text):
                            cfg = {"perfil_nome": f"esp_{var_canal}", "user_agent": USER_AGENT, "referer": f"{BASE_SELENIUM}/{var_canal}", "cookie": cookie_str}
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                stream_url = match.group(1) if match else None
                if not stream_url:
                    match = re.search(r'source\s*:\s*["\']([^"\']+\.m3u8[^"\']*)', resp.text)
                    stream_url = match.group(1) if match else None
                if stream_url:
                    with LOCK_SELENIUM:
                        CACHE_SELENIUM[var_canal] = (stream_url, "", time.time())
                    try:
                        headers = {"User-Agent": USER_AGENT, "Referer": url_pagina}
                        r = SESSAO.get(stream_url, headers=headers, timeout=10, verify=False)
                        if r.status_code == 200 and ("#EXTM3U" in r.text or "#EXT-X" in r.text):
                            cfg = {"perfil_nome": f"esp_{var_canal}", "user_agent": USER_AGENT, "referer": url_pagina, "cookie": ""}
                            return r, stream_url, cfg, "chrome120"
                    except Exception: pass
        except Exception: pass
    return None, None, None, None
...
@app.route('/')
def raiz():
    return Response(gerar_lista_m3u(), mimetype='application/vnd.apple.mpegurl')
...
if __name__ == '__main__':
    ...
    print(f"Lista M3U: {URL_PUBLICA}/lista.m3u")
